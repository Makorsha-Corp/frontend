// src/legacy-reference/rbac/AccessControlService.ts
import { supabase_client } from "../legacy-types";
import { AccessControl, AccessType, AccessRole, Status } from "../legacy-types";
...
export async function listStatuses(): Promise<Status[]> {
  const { data, error } = await (supabase_client as any)
    .from("statuses")
    .select("id,name")
...
export async function grantPageAccess(role: AccessRole, page: string) {
  const { error } = await (supabase_client as any)
    .from("access_control")
...
export async function setPageRoles(page: string, roles: AccessRole[]) {
  ...
  const current = new Set<AccessRole>((data ?? []).map((d: any) => (d as AccessControl).role));
  ...
  if (toAdd.length) {
    ...
    const { error: e1 } = await (supabase_client as any)
      .from("access_control")
      .upsert(rows, { onConflict: "type,target,role", ignoreDuplicates: true });
    if (e1) throw e1;
  }
  if (toRemove.length) {
    const { error: e2 } = await (supabase_client as any)
      .from("access_control")
      .delete()
      .eq("type", "page" as AccessType)
      .eq("target", page)
...
export async function fetchFeatureRoles(key: FeatureKey): Promise<AccessRole[]> {
  ...
  if (error) throw error;
  return (data ?? []).map((d: any) => (d as AccessControl).role as AccessRole);
}
...
export interface RoleAccessSnapshot {
  pagesSet: Set<string>;
  manageOrderSet: Set<number>;
  featuresSet: Set<FeatureKey>;
}
...
export async function fetchRoleAccessSnapshot(role: AccessRole): Promise<RoleAccessSnapshot> {
  ...
  const pagesSet = new Set<string>((pagesRes.data ?? []).map((d: any) => (d as AccessControl).target));

  const manageOrderSet = new Set<number>(
    (moRes.data ?? [])
      .map((d: any) => Number((d as AccessControl).target))
  ...
  const featuresSet = new Set<FeatureKey>(
    (featRes.data ?? [])
      .map((d: any) => (d as AccessControl).target)
      .filter((t: any): t is FeatureKey => validFeatures.has(t as FeatureKey))
  );
