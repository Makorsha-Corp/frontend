...
import type { SalesOrderApprover } from '@/types/salesOrder';
import type { WorkspaceMember } from '@/types/workspace';

export interface ManageSoApprovalsDialogProps {
  open: boolean;
...
const ManageSoApprovalsDialog: React.FC<ManageSoApprovalsDialogProps> = ({
  open,
  onOpenChange,
  approvers,
  assignableMembers,
    ...
    <DialogContent className="flex h-[66vh] max-h-[66vh] w-[min(42rem,94vw)] max-w-none flex-col gap-0 overflow-hidden p-0 sm:max-w-none">
      ...
      <div className="flex flex-1 min-h-0 flex-col gap-4 overflow-y-auto px-6 py-4">
        <div className="flex items-center justify-between gap-3">
          <Label htmlFor="so-required-approvals" className="text-sm text-muted-foreground">
            Required approvals
          </Label>
          <Input
            id="so-required-approvals"
            type="number"
            min={0}
        ...
        {assignableMembers.length > 0 && (
          <div className="flex items-center gap-2">
            ...
            <Select value="" disabled={disabled} onValueChange={(v) => onAddApprover(Number(v))}>
        ...
        <div className="space-y-2">
          <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Assigned approvers</p>
          {approvers.length === 0 ? (
                ...
                <div key={a.id} className="flex items-center justify-between gap-2 rounded-lg border border-border px-3 py-2">
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium text-card-foreground">{a.user_name ?? `User #${a.user_id}`}</p>
                      <p className="truncate text-xs text-muted-foreground">{a.user_position || a.user_email || ''}</p>
                    </div>
                  </div>
...
export default ManageSoApprovalsDialog;
