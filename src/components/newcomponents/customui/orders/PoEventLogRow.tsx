...
import { PO_EVENT_VISUALS } from './purchaseOrderEventVisuals';
...
interface PoEventLogRowProps {
  event: PurchaseOrderEvent;
  isLast: boolean;
}

const PoEventLogRow: React.FC<PoEventLogRowProps> = ({ event, isLast }) => {
  const [open, setOpen] = useState(false);
  const ev = PO_EVENT_VISUALS[event.event_type] ?? PO_EVENT_VISUALS.default;
  const Icon = ev.icon;
