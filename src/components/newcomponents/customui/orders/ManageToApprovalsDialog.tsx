...
export interface ManageToApprovalsDialogProps {
  open: boolean;
...
const ManageToApprovalsDialog: React.FC<ManageToApprovalsDialogProps> = ({
  open,
  onOpenChange,
  approvers,
  assignableMembers,
  ...
  <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="flex h-[66vh] max-h-[66vh] w-[min(42rem,94vw)] max-w-none flex-col gap-0 overflow-hidden p-0 sm:max-w-none">
      <DialogHeader className="shrink-0 space-y-1 border-b border-border px-6 py-4">
        ...
        <DialogDescription>
          Add workspace members as approvers and set how many approvals are required.
        </DialogDescription>
      </DialogHeader>

      <div className="flex flex-1 min-h-0 flex-col gap-4 overflow-y-auto px-6 py-4">
        <div className="flex items-center justify-between gap-3">
          <Label htmlFor="tr-required-approvals" className="text-sm text-muted-foreground">
            Required approvals
          </Label>
          <Input
            id="tr-required-approvals"
            type="number"
            min={0}
...
export default ManageToApprovalsDialog;
