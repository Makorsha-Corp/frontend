...
import { formatKpiActionStripSummary } from './kpiMetricsText';
import type { OrdersOverviewKpiProps } from './kpiSectionTypes';

const KpiActionStrip: React.FC<OrdersOverviewKpiProps> = ({ stats, isLoading }) => {
  const alerts = [
  ...
  return (
    <div className="space-y-2 pt-1">
      <p className="text-sm text-muted-foreground">{formatKpiActionStripSummary(stats)}</p>
      {alerts.length > 0 ? (
