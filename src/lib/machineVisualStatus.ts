...
export function getMachineVisualKind(machine: Machine): MachineVisualKind {
  ...
  if (machine.is_running || machine.latest_status_type === 'RUNNING') return 'running';
...
export const machineTopBarClass: Record<MachineVisualKind, string> = {
  ...
  idle: 'bg-brand-primary/70 dark:bg-brand-primary/55',
  off: 'bg-red-500/65 dark:bg-red-500/50',
...
export const machineBadgeClass: Record<MachineVisualKind, string> = {
  ...
  idle: 'border border-brand-primary/40 bg-brand-primary/15 text-brand-primary',
  off: 'border border-red-500/40 bg-red-500/15 text-red-700 dark:text-red-400',
  maintenance:
...
export const machineStatusSegmentClass: Record<MachineVisualKind, string> = {
  ...
  idle: 'bg-brand-primary/80 dark:bg-brand-primary/65',
  off: 'bg-red-500/75 dark:bg-red-500/60',
...
export const machineKpiValueClass: Record<MachineVisualKind, string> = {
  ...
  idle: 'text-brand-primary',
  off: 'text-red-700 dark:text-red-400',
...
export const statusMetricTileClass: Record<MachineVisualKind, string> = {
  ...
  idle: `${dashIconTileBase} border border-brand-primary/40 bg-brand-primary/15`,
  off: `${dashIconTileBase} border border-red-500/40 bg-red-500/15`,
...
export const statusMetricIconClass: Record<MachineVisualKind, string> = {
  ...
  idle: 'h-5 w-5 text-brand-primary',
  off: 'h-5 w-5 text-red-700 dark:text-red-400',
...
export const machineSelectorTileClass = (kind: MachineVisualKind, isRowHighlighted: boolean) => {
  ...
  switch (kind) {
    ...
    case 'idle':
      return 'border-brand-primary/25 ring-1 ring-brand-primary/10';
    case 'maintenance':
...
export function activeEventButtonClass(type: MachineEventType): string {
  switch (type) {
    case 'RUNNING':
      return 'border-emerald-500/40 bg-emerald-500/15 text-emerald-700 hover:bg-emerald-500/20 hover:!text-emerald-700 dark:text-emerald-400 dark:hover:!text-emerald-400';
    case 'MAINTENANCE':
      return 'border-amber-500/45 bg-amber-500/15 text-amber-800 hover:bg-amber-500/20 hover:!text-amber-800 dark:text-amber-400 dark:hover:!text-amber-400';
    case 'IDLE':
      return 'border-brand-primary/40 bg-brand-primary/15 text-brand-primary hover:bg-brand-primary/20 hover:!text-brand-primary';
    case 'OFF':
      return 'border-red-500/40 bg-red-500/15 text-red-700 hover:bg-red-500/20 hover:!text-red-700 dark:text-red-400 dark:hover:!text-red-400';
    default:
      return '';
  }
}
