...
import { Wrench, Plus, Loader2, Search, CalendarIcon, PanelLeft } from 'lucide-react';
...
const WorkOrdersPage: React.FC = () => {
  ...
  const filteredOrders = useMemo(
    () => filterWorkOrders(orders, filterOpts, labelCtx),
    [orders, filterOpts, labelCtx]
  );
    ...
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        ...
        <div className="shrink-0 border-b border-border bg-card/50 px-4 py-3 flex flex-wrap items-center gap-2">
          ...
          <Select
            value={statusFilter}
            onValueChange={(v) => setStatusFilter(v as WorkOrderStatusFilter)}
          >
          ...
          <Select value={workTypeFilter} onValueChange={(v) => setWorkTypeFilter(v as WorkTypeFilter)}>
          ...
          <div className="flex-1 min-w-0 min-h-0 overflow-hidden">
            {selectedOrder ? (
              <WorkOrderDetailPanel
                ...
                onStatusUpdated={() => setSelectedOrder(selectedOrder.id)}
              />
            ) : (
